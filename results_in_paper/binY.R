...
sim.size = function(tau, pi, size = 0.05)
{
  ...
  for(i in 1:m)
  {
    #w/o confounders
    obj1 = lm(M[i,] ~  X )
    obj2 = glm(Y[i,] ~ M[i,] + X, family = binomial)
  ...
  res <- tryCatch({
    ...
    list(fdr_mlfdr = fdr_mlfdr, pow_mlfdr = pow_mlfdr)
  }, error = function(e) {
    fdr_mlfdr = NA
    pow_mlfdr = NA
    list(fdr_mlfdr = fdr_mlfdr, pow_mlfdr = pow_mlfdr)
  })
  fdr_mlfdr <- res$fdr_mlfdr
  pow_mlfdr <- res$pow_mlfdr
...
beepr::beep(4)
write.csv(sim.res, paste0("results/mean_m",m,"n",n,"sparse_binY.csv"))
write.csv(sim.se, paste0("results/se_m",m,"n",n,"sparse_binY.csv"))
sim.res
...
for (k1 in 1:length(tau)) {
  ...
  #write this bit in a loop
  # temp = matrix(nrow = n.sim, ncol = 6)
  # for (s in 1:n.sim){
  #   temp[s, ] = sim.size(tau[k1], pi)
  # }
...
write.csv(sim.res, paste0("results/mean_m",m,"n",n,"dense_binY.csv"))
write.csv(sim.se, paste0("results/se_m",m,"n",n,"dense_binY.csv"))
