...
HDMT_control_DR_adjust = function(p.M,p.Y,estws,significance_upper,control.method){
  sim.num <- length(p.M)
  ...
  ss <- DR_HDMT_thr_adjust(p.M, p.Y,pi.10.est,pi.01.est,pi.00.est,significance_upper,control.method)
  
  return(ss)
}
DR_HDMT_thr_adjust <- function(p.M, p.Y,pi.10.est,pi.01.est,pi.00.est,significance_upper,control.method){
  sim.num <- length(p.M)
  c <- pi.01.est + pi.10.est + pi.00.est
  pi.11.est <- max(1 - c, 0)
